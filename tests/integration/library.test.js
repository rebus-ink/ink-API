...
const test = async () => {
  ...
  const createPublicationRequest = number => {
  ...
  await tap.test('filter library by collection', async () => {
    ...
    const pubBres = await request(app)
      ...
      .send(createPublicationRequest(1))
    ...
    // publication C
    const pubcres = await request(app)
      .post(`${userUrl}/activity`)
      .set('Host', 'reader-api.test')
      .set('Authorization', `Bearer ${token}`)
      .type(
      ...
      .send(createPublicationRequest(2))
    ...
    await tap.ok(Array.isArray(body.items))
    // documents should include:
    await tap.equal(body.items[0].name, 'Publication 1')
  })
