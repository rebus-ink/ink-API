...
const test = async app => {
  ...
  await tap.test('Source addTag', async () => {
    ...
    await tap.equal(res.sourceId, source.id)
  ...
  await tap.test('addTagToSource with invalid tag id ', async () => {
    let error
    try {
      await Source_Tag.addTagToSource(urlToId(source.id), createdTag.id + '123')
    } catch (err) {
      error = err
    }
    await tap.ok(error)
  ...
  await tap.test('addTagToSource with invalid source id ', async () => {
    let error
    try {
      await Source_Tag.addTagToSource(undefined, createdTag.id)
    } catch (err) {
      error = err
    }
    await tap.ok(error)
  ...
  await tap.test('removeTagFromSource with invalid tag id ', async () => {
    let error
    try {
      await Source_Tag.removeTagFromSource(
        urlToId(source.id),
        createdTag.id + '123'
      )
    } catch (err) {
  ...
  await tap.test('removeTagFromSource with invalid source id ', async () => {
    let error
    try {
      await Source_Tag.removeTagFromSource('123', createdTag.id)
    } catch (err) {
      error = err
    }
  ...
  //   await Source_Tag.addTagToSource(source.id, createdTag.id)
  ...
  await tap.test(
    'Delete Source_Tags of a Source with an invalid id',
    async () => {
