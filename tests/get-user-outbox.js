// test route to GET a user outbox
...
const main = async () => {
  ...
  await tap.test('GET /<userid>/activity with no authentication', async () => {
  ...
  await tap.test('GET /<userid>/activity as other user', async () => {
    const options = {
      subject: 'bar',
      expiresIn: '24h',
  ...
  await tap.test(
    'GET /<userid>/activity with correct authentication',
    async () => {
      const options = {
        subject: 'foo',
        expiresIn: '24h',
      ...
      await tap.equal(body.id, 'https://reader-api.test/foo/activity')
    }
  )
}
